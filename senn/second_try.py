...
song_sound_tut = delayperm(S.dot(song_ra_tut),tau); 
...
Q=A.dot(S); # Total motor to auditory transformation, eg song_aud_tut=delayperm(Q*song_ra_tut,tau)
...
for i in xrange(0,n_learn):
  ...
  minv=delayperm(w_lman.dot(aud_soma),0); 
  ...
  # of the trace of m.
  dm=(delayperm(ra_soma,tau+0)-minv); 
  ...
  dw_lman=dm.dot(delayperm(aud_soma,0).T); 
  ...
  e_lman[i]=(sum(dm*dm))/(T*n_ra); # change from dot product? Probably not.
...
minv=delayperm(w_lman.dot(song_aud_tut),0); 
...
for i in xrange(0, nlearn_pred):
  ...
  dwpred=dm.dot(song_aud_tut_del.T); # dot product? Transponed?
